...
The native rewrite that this section originally called for is done, and the surface has since spread to the phone and the iPad with widgets on both screens. What has not moved is the engine: it is still Python, still tied to the one Mac that holds the credential, and the Pi that now serves every other device only ever relays what that Mac has already worked out. Porting the engine to Swift would remove the last single point of failure in the chain.
